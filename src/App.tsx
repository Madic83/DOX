  ...
  if (showVitals) {
    ...
    //   if (data.length === 0) return null
      ...
      const points = data.map((reading, index) => {
        const value = getNumericValue(reading[field] as string)
        const x = padding + (index / Math.max(data.length - 1, 1)) * graphWidth
        const y = padding + graphHeight - ((value - min) / (max - min)) * graphHeight
        return { x, y, value, reading }
      })
      ...
      return (
        <div style={{ background: '#111', padding: '20px', borderRadius: '8px', border: '1px solid #333', marginBottom: '20px' }}>
          <h4 style={{ marginBottom: '15px', color: '#F3D021' }}>
            {field === 'pulse' && 'Puls (Hjärtfrekvens)'}
            {field === 'respiration' && 'Andning (Andningsfrekvens)'}
            {field === 'spo2' && 'SpO2 (Syremättnad)'}
            {field === 'temperature' && 'Temperatur'}
          </h4>
          <div style={{ overflowX: 'auto' }}>
            <svg width={Math.max(width, 600)} height={height} style={{ background: '#0a0a0a', borderRadius: '4px' }}>
              {/* Rutnät - vertikala linjer */}
              {Array.from({ length: data.length }).map((_, i) => {
                const x = padding + (i / Math.max(data.length - 1, 1)) * graphWidth
                return <line key={`vgrid-${i}`} x1={x} y1={padding} x2={x} y2={height - padding} stroke="#333" strokeDasharray="5,5" strokeWidth="1" />
              })}
              
              {/* Rutnät - horisontella linjer */}
              {Array.from({ length: 5 }).map((_, i) => {
                ...
                return (
                  <g key={`hgrid-${i}`}>
                    ...
                    <text x={padding - 10} y={y + 4} textAnchor="end" fill="#888" fontSize="12">
                      {Math.round(value)}
                    </text>
                  </g>
                )
              ...
              {points.map((p, i) => {
                ...
                if (field === 'pulse') {
                  dotColor = value > 120 ? '#f59e0b' : value < 50 ? '#dc2626' : '#10b981'
                } else if (field === 'respiration') {
                  dotColor = value > 25 ? '#f59e0b' : value < 12 ? '#dc2626' : '#10b981'
                } else if (field === 'spo2') {
                  dotColor = value < 94 ? '#dc2626' : value < 98 ? '#f59e0b' : '#10b981'
                } else if (field === 'temperature') {
                  dotColor = value > 38.5 ? '#f59e0b' : value < 36.5 ? '#dc2626' : '#10b981'
                }

                return (
                  <g key={`point-${i}`}>
                    <circle cx={p.x} cy={p.y} r="6" fill={dotColor} stroke="#000" strokeWidth="2" />
                    <text x={p.x} y={p.y - 15} textAnchor="middle" fill={dotColor} fontSize="14" fontWeight="bold">
                      {value}
                    </text>
                    <text x={p.x} y={height - padding + 25} textAnchor="middle" fill="#888" fontSize="11">
                      {p.reading.time === 'CURRENT' ? 'NU' : p.reading.time}
                    </text>
                  </g>
                )
              })}
